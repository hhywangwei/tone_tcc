package com.tcc.cti.web.common.page;
...
public class Page {
	private int showCount = Const.pagesize; //每页显示记录数
	...
	public String getPageStr() {
		...
		if(totalResult>0){
			...
			for(int i=0;i<totalPage;i++){
				if(currentPage>i+1-5 && currentPage<i+1+5){
					   list.add(i+1);
				}
			}
			startTag = list.get(0);
			endTag = list.get(list.size()-1);
			
			for(int i=startTag; i<=totalPage && i<=endTag; i++){
				if(currentPage==i)
					sb.append("<li class=\"currents\">"+i+"</li>\n");
				else
